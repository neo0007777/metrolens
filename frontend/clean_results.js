...
// The file is corrupted starting from `{/* TAB 3: DATA EXTRACTED */}` down to where the actual `)}` should be.
// Let's find the START of TAB 3:
const tab3Start = code.indexOf('{/* TAB 3: DATA EXTRACTED */}');

// The end of the Data tab logic is exactly where `</main>` begins.
const mainEnd = code.indexOf('</main>');

// We will replace everything from `tab3Start` up to `mainEnd` with the correct Data Tab code.
const properDataTab = `{/* TAB 3: DATA EXTRACTED */}
        {activeTab === 'data' && (
...
code = code.substring(0, tab3Start) + properDataTab + code.substring(mainEnd);
